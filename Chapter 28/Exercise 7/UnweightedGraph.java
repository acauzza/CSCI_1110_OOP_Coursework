  ...
  public List<Integer> getACycle() { 
	  List<Integer> allVertices = new ArrayList<>();//create a list with all the vertices
	    ...
	    int[] parent = new int[vertices.size()];
	    	...
	    	parent[i] = -1; //create an int array to determine parent of vertex (-1 by default)
	    ...
	    while (allVertices.size() > 0) { //there are unvisited vertices, check list size for this 
	        ...
	        while (!stack.isEmpty()) { //the stack is not empty 
	            ...
	            if (neighbors.get(x).size() == 0) { //x has no neighbors
	                stack.pop();
	                continue;//pop a vertex from the stack;
	            }
	            else {
	                for (int i = neighbors.get(x).size() - 1; i >= 0; i--) { //loop through all neighbors of x with loop variable i
	                Edge e = neighbors.get(x).get(i); //Let e be the edge between x and the neighbor; 
	                   ...
	                   if (!isVisited[e.v]) { //ending vertex of e (e.v) is not visited 
	                       ...
	                       List <Integer> list = new ArrayList<>();//create an Integer ArrayList;
	                       list.add(e.v);//add e.v to list;
	                       while (x != e.v && x != -1) { //x != e.v and x != -1
